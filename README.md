...
as they can be accessed by the provider as described below.
...
Additional climatological variables created by Dr. Abatzoglou for this
analysis, used to calculate the absolute daily minimum temperature.
These are found in the archived files named `absmin*.nc`.

-   the variable `cadj` representing the perturbation from true coldest
    month to climatological coldest month.
-   the the variable `adjustC`, which represents the difference in
...
](http://chart.apis.google.com/chart?cht=tx&chl=%0A%5Ctextrm%7Babsmin%7D%20%3D%20%5Ctextrm%7Btmin%7D%20-%20%5Ctextrm%7Bcadj%7D%20-%5Ctextrm%7BadjustC%7D%0A "
\textrm{absmin} = \textrm{tmin} - \textrm{cadj} -\textrm{adjustC}
")
...
\\end{matrix}\\right.
](http://chart.apis.google.com/chart?cht=tx&chl=%0A%5Calpha%3D%0A%5Cleft%5C%7B%5Cbegin%7Bmatrix%7D%0A0%2C%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%26%20%5Ctextrm%7Bfor%20%7D%20I%3C%200.6252%20%5C%5C%20%0A-7%5Ctimes10%5E%7B-7%7DI%5E2%2B0.0016%5C%3BI-0.0001%2C%20%26%20%5Ctextrm%7Bfor%20%7D%20I%3E%200.6252%20%0A%5Cend%7Bmatrix%7D%5Cright.%0A "
\alpha=
\left\{\begin{matrix}
0,                                   & \textrm{for } I< 0.6252 \\ 
-7\times10^{-7}I^2+0.0016\;I-0.0001, & \textrm{for } I> 0.6252 
...
Where ![I](http://chart.apis.google.com/chart?cht=tx&chl=I "I") is the
daily photosynthetically active radiation for the month.
...
![\\textrm{EPI}\_\\textrm{month}=\\alpha\\times\\beta\\times\\Gamma](http://chart.apis.google.com/chart?cht=tx&chl=%5Ctextrm%7BEPI%7D_%5Ctextrm%7Bmonth%7D%3D%5Calpha%5Ctimes%5Cbeta%5Ctimes%5CGamma "\textrm{EPI}_\textrm{month}=\alpha\times\beta\times\Gamma")

**Five year total EPI** is the sum of monthly EPIs multiplied by five:

![\\textrm{EPI}\_\\textrm{5 years}=5\\times\\sum\_{0}^{12}\\textrm{EPI}\_\\textrm{month}](http://chart.apis.google.com/chart?cht=tx&chl=%5Ctextrm%7BEPI%7D_%5Ctextrm%7B5%20years%7D%3D5%5Ctimes%5Csum_%7B0%7D%5E%7B12%7D%5Ctextrm%7BEPI%7D_%5Ctextrm%7Bmonth%7D "\textrm{EPI}_\textrm{5 years}=5\times\sum_{0}^{12}\textrm{EPI}_\textrm{month}")

**Predicted Biomass over a 5 year cropping cycle** uses the parameters
from

![\\textrm{Biomass}=1.7607\\times\\textrm{EPI}-14.774](http://chart.apis.google.com/chart?cht=tx&chl=%5Ctextrm%7BBiomass%7D%3D1.7607%5Ctimes%5Ctextrm%7BEPI%7D-14.774 "\textrm{Biomass}=1.7607\times\textrm{EPI}-14.774")

**Simulating Rock Mulch**

![\\beta\_{\\textrm{rock mulch}}=\\beta \* 1.83](http://chart.apis.google.com/chart?cht=tx&chl=%5Cbeta_%7B%5Ctextrm%7Brock%20mulch%7D%7D%3D%5Cbeta%20%2A%201.83 "\beta_{\textrm{rock mulch}}=\beta * 1.83")
...
Given
![\\beta = 0.0279\\times\\textrm{P}-0.2851](http://chart.apis.google.com/chart?cht=tx&chl=%5Cbeta%20%3D%200.0279%5Ctimes%5Ctextrm%7BP%7D-0.2851 "\beta = 0.0279\times\textrm{P}-0.2851"),
find the values of
![P](http://chart.apis.google.com/chart?cht=tx&chl=P "P") where
![\\beta\\geq 1](http://chart.apis.google.com/chart?cht=tx&chl=%5Cbeta%5Cgeq%201 "\beta\geq 1")?

![\\textrm{P}\_{\\beta=1}=\\frac{1+0.2851}{0.0279}\\simeq= 46.06093](http://chart.apis.google.com/chart?cht=tx&chl=%5Ctextrm%7BP%7D_%7B%5Cbeta%3D1%7D%3D%5Cfrac%7B1%2B0.2851%7D%7B0.0279%7D%5Csimeq%3D%2046.06093 "\textrm{P}_{\beta=1}=\frac{1+0.2851}{0.0279}\simeq= 46.06093")

-   for each month if
    -   ![\\alpha &gt; 0](http://chart.apis.google.com/chart?cht=tx&chl=%5Calpha%20%3E%200 "\alpha > 0")
        AND
    -   ![\\gamma &gt; 0](http://chart.apis.google.com/chart?cht=tx&chl=%5Cgamma%20%3E%200 "\gamma > 0")
        AND
    -   ![\\textrm{P} &lt; 46.061\\textrm{mm}](http://chart.apis.google.com/chart?cht=tx&chl=%5Ctextrm%7BP%7D%20%3C%2046.061%5Ctextrm%7Bmm%7D "\textrm{P} < 46.061\textrm{mm}")
-   calculate irrigation required = water demand
    ![46.01 - P](http://chart.apis.google.com/chart?cht=tx&chl=46.01%20-%20P "46.01 - P")
-   annual irrigation demand =
    ![\\sum\_{jan}^{dec}46.01 - \\textrm{P}](http://chart.apis.google.com/chart?cht=tx&chl=%5Csum_%7Bjan%7D%5E%7Bdec%7D46.01%20-%20%5Ctextrm%7BP%7D "\sum_{jan}^{dec}46.01 - \textrm{P}")
...
*mostly provided inline with text*
